...
if USE_SHEETS and Path(CREDENTIALS_FILE).exists():
    ...
    # Ensure headers in correct order
...
def save_query_answer(query, answer):
    ...
    if USE_SHEETS and 'sheet' in globals():
        ...
        sheet.append_row(["", query, timestamp, "", "", "", "", answer, "N/A"])
...
def categorize_ticket(content):
    prompt = f"Categorize this ticket into product_maintenance, billing, or support:\n\n{content}"
    try:
...
def process_ticket(ticket, retriever, prompt):
    ticket['ticket_category'] = categorize_ticket(ticket['ticket_content'])
    ...
    if USE_SHEETS and 'sheet' in globals():
        ...
        print(f"✅ Ticket {ticket['ticket_id']} saved with solution.")

    return ticket

def update_ticket_status(ticket_id, new_status):
    """Update ticket status (Open/In Progress/Closed/Resolved)."""
    if USE_SHEETS and 'sheet' in globals():
