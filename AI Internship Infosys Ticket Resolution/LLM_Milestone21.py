...
if USE_SHEETS:
    try:
        ...
        # ✅ Open spreadsheet by name and worksheet "Sheet1"
        ...
        st.success(f"✅ Connected to Google Sheets → {WORKSHEET_NAME}")

    except gspread.WorksheetNotFound:
        st.error(f"🚨 Worksheet '{WORKSHEET_NAME}' not found in your spreadsheet. Please create it manually.")
        USE_SHEETS = False
        sheet = None
...
if not USE_SHEETS:
    if not QUERIES_FILE.exists():
        with open(QUERIES_FILE, "w", encoding="utf-8") as f:
            json.dump({"headers": HEADERS, "data": []}, f, indent=2)
    st.warning("📂 Using local JSON fallback for queries.")


# --- Ticket Save Function ---
def save_ticket(ticket: Dict):
    """Save a ticket to Google Sheets if available, otherwise to local JSON."""
    global USE_SHEETS, sheet
    ...
    if USE_SHEETS and sheet:
        try:
            ...
            st.error(f"⚠️ Failed to save to Google Sheets, falling back to JSON: {e}")
            USE_SHEETS = False  # switch permanently
            return save_ticket(ticket)  # retry with JSON fallback
